  ...
  persist(
    (set, get) => ({
      ...
      logout: () => {
        ...
        try {
          localStorage.removeItem('moei-chat-auth')
          localStorage.removeItem('moei_current_agent')
          sessionStorage.removeItem('szhp-auth-storage')
        } catch { /* ignore in SSR */ }
        // Notify other components (chat widget, customer portal, etc.)
        try {
