/**
 * API Authentication & Authorization for Hono Worker
 * Validates API keys and admin tokens on mutation endpoints
 * Adapted from src/lib/api-auth.ts for Hono middleware
 */
 ...
 */
export function requireMutationAuth(request: Request): { status: number; body: Record<string, unknown> } | null {
 ...
 */
export function checkPublicRateLimit(request: Request): { status: number; body: Record<string, unknown> } | null {
  const clientId = getClientId(request)
...
export function validateRequestBody(
  ...
  rules: Record<string, { required?: boolean; type?: string; maxLength?: number }>
