...
import { calculateDBR as calculateDBRShared } from '@/lib/business/dbr'
...
export function CustomerPortal() {
  ...
  const calculateDBR = () => {
    ...
    return calculateDBRShared(installment, income)
  }
                        ...
                        <div key={req.id} className="flex items-center justify-between p-3 rounded-lg bg-ae-black-50">
                          <div>
                            ...
                            <div className="text-xs text-ae-black-400">
                              {new Date(req.createdAt).toLocaleString(isAr ? 'ar-AE' : 'en-US', { month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' })}
                            </div>
                          </div>
                          ...
                          <motion.div key="step4" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-4">
                            ...
                            <div className="space-y-3">
                              {Object.entries({
                                ...
                                [t('form.monthlyIncome', language)]: formData.monthlyIncome ? formatAED(parseFloat(formData.monthlyIncome)) : '',
                                ...
                                [t('form.remainingBalance', language)]: formData.remainingBalance ? formatAED(parseFloat(formData.remainingBalance)) : '',
                                [t('form.monthlyInstallment', language)]: formData.monthlyInstallment ? formatAED(parseFloat(formData.monthlyInstallment)) : '',
                        ...
                        <div className="flex items-center justify-between">
                          <div className="flex-1 min-w-0">
                            ...
                            <div className="flex items-center gap-4 mt-2 text-xs text-ae-black-400">
                              ...
                              <span>{t('portal.submittedOn', language)}: {new Date(req.createdAt).toLocaleString(isAr ? 'ar-AE' : 'en-US', { month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' })}</span>
                            </div>
                          </div>
