...
import {
  ...
  Loader2, AlertCircle, History, Info, Search, SlidersHorizontal, X
} from 'lucide-react'
...
export function DataImport() {
  ...
  const [historyFileType, setHistoryFileType] = useState('all')
  ...
  const filteredHistory = useMemo(() => {
    return importHistory.filter((entry) => {
      // Search filter
      if (historySearch.trim()) {
        const q = historySearch.toLowerCase()
        if (!entry.fileName.toLowerCase().includes(q)) return false
      }
      // File type filter
      if (historyFileType !== 'all') {
        const ext = entry.fileName.toLowerCase().split('.').pop() || ''
        if (historyFileType === 'csv' && ext !== 'csv') return false
        if (historyFileType === 'xlsx' && ext !== 'xlsx') return false
      }
      // Status filter
      if (historyStatus === 'success' && entry.errors > 0) return false
      if (historyStatus === 'failed' && entry.errors === 0) return false
      if (historyStatus === 'partial' && (entry.errors === 0 || entry.skipped === 0)) return false
      // Table filter
      if (historyTable !== 'all' && entry.targetTable !== historyTable) return false
      // Date filter
      if (historyDateFrom) {
  ...
  }, [importHistory, historySearch, historyFileType, historyStatus, historyTable, historyDateFrom, historyDateTo])

  // History active filter count
      ...
      <CardHeader className="pb-3">
        <div className="flex items-center justify-between">
          ...
          {historyFilterCount > 0 && (
            <Button variant="ghost" size="sm" className="text-xs h-6 px-2 text-muted-foreground" onClick={clearHistoryFilters}>
      ...
      <CardContent className="space-y-3">
        ...
        <div className="flex flex-col sm:flex-row gap-2">
          <div className="relative flex-1">
            ...
            <Input
              ...
              placeholder={isAr ? 'بحث بالاسم...' : 'Search by file name...'}
              className="ps-8 pe-8 h-8 text-xs"
            />
            {historySearch && (
              <button type="button" className="absolute end-3 top-1/2 -translate-y-1/2 text-muted-foreground hover:text-foreground" onClick={() => setHistorySearch('')}>
                <X className="w-3.5 h-3.5" />
              </button>
            )}
          </div>
          <Select value={historyFileType} onValueChange={setHistoryFileType}>
            <SelectTrigger className="w-full sm:w-[120px] h-8 text-xs">
              <SelectValue placeholder={isAr ? 'نوع الملف' : 'File type'} />
            </SelectTrigger>
            <SelectContent>
          ...
          <Select value={historyStatus} onValueChange={setHistoryStatus}>
            ...
            <SelectContent>
              ...
              <SelectItem value="partial">{isAr ? 'جزئي' : 'Partial'}</SelectItem>
            </SelectContent>
          </Select>
          <Select value={historyTable} onValueChange={setHistoryTable}>
            <SelectTrigger className="w-full sm:w-[160px] h-8 text-xs">
              <SelectValue placeholder={isAr ? 'الجدول' : 'Table'} />
            </SelectTrigger>
            <SelectContent>
              <SelectItem value="all">{isAr ? 'جميع الجداول' : 'All tables'}</SelectItem>
              {TARGET_TABLES.map(table => (
                <SelectItem key={table.value} value={table.value}>{isAr ? table.ar : table.en}</SelectItem>
              ))}
            </SelectContent>
          </Select>
        </div>
        <div className="flex items-center gap-2 flex-wrap">
          <Label className="text-xs text-muted-foreground whitespace-nowrap">{isAr ? 'تاريخ' : 'Date'}</Label>
          ...
          {(historyDateFrom || historyDateTo) && (
            <Button variant="ghost" size="sm" className="text-xs h-7 px-2 text-muted-foreground hover:text-foreground" onClick={() => { setHistoryDateFrom(''); setHistoryDateTo('') }}>
        ...
        {historyFilterCount > 0 && (
          <div className="flex items-center gap-1.5 text-xs flex-wrap">
            ...
            <span className="text-muted-foreground">
              {isAr ? `${filteredHistory.length} من ${importHistory.length} سجل` : `${filteredHistory.length} of ${importHistory.length} records`}
            </span>
            {historyFileType !== 'all' && (
              <Badge variant="secondary" className="text-[10px] gap-0.5 h-5 uppercase">{historyFileType}<X className="w-2.5 h-2.5 cursor-pointer" onClick={() => setHistoryFileType('all')} /></Badge>
            )}
            {historyStatus !== 'all' && (
              <Badge variant="secondary" className="text-[10px] gap-0.5 h-5">{historyStatus === 'success' ? (isAr ? 'ناجح' : 'Success') : historyStatus === 'failed' ? (isAr ? 'فاشل' : 'Failed') : (isAr ? 'جزئي' : 'Partial')}<X className="w-2.5 h-2.5 cursor-pointer" onClick={() => setHistoryStatus('all')} /></Badge>
            )}
            {historyTable !== 'all' && (
              <Badge variant="secondary" className="text-[10px] gap-0.5 h-5">{getTableLabel(historyTable)}<X className="w-2.5 h-2.5 cursor-pointer" onClick={() => setHistoryTable('all')} /></Badge>
            )}
            {historyDateFrom && (
              <Badge variant="secondary" className="text-[10px] gap-0.5 h-5">{isAr ? 'من' : 'From'}: {historyDateFrom}<X className="w-2.5 h-2.5 cursor-pointer" onClick={() => setHistoryDateFrom('')} /></Badge>
            )}
            {historyDateTo && (
              <Badge variant="secondary" className="text-[10px] gap-0.5 h-5">{isAr ? 'إلى' : 'To'}: {historyDateTo}<X className="w-2.5 h-2.5 cursor-pointer" onClick={() => setHistoryDateTo('')} /></Badge>
            )}
          </div>
        )}
        ...
        {importHistory.length === 0 ? (
          ...
          <div className="text-center py-6">
            ...
            <p className="text-sm text-gray-500">{isAr ? 'لا توجد نتائج' : 'No results found'}</p>
            <Button variant="ghost" size="sm" className="text-xs mt-1 text-muted-foreground" onClick={clearHistoryFilters}>
