...
import {
  ...
  Cloud, Monitor, Cpu, Calendar, X, SlidersHorizontal
} from 'lucide-react'
...
const PROVIDER_CONFIG: Record<string, { label: string; labelAr: string; icon: React.ElementType; color: string; bgColor: string; borderColor: string; needsApiKey: boolean }> = {
  ...
  zai: { label: 'Z.ai Cloud', labelAr: 'Z.ai السحابي', icon: Cloud, color: 'text-orange-600', bgColor: 'bg-orange-50', borderColor: 'border-orange-200', needsApiKey: false },
}
  ...
  const handleSave = async () => {
    ...
    try {
      const payload = {
        ...
        capabilities: JSON.stringify(editingModel.capabilities),
      ...
      if (success) {
        // If isDefault was toggled ON, also set as default LLM/VLM
        if (editingModel.isDefault) {
          const caps = (editingModel.capabilities as string[]) || []
          const savedModelId = editingModel.id || ''
          if (savedModelId) {
            // Set as default LLM
            await handleSetDefaultModel({ ...editingModel, id: savedModelId } as ModelConfig, 'llm')
            // If has vision capability, also set as default VLM
            if (caps.includes('vision')) {
              await handleSetDefaultModel({ ...editingModel, id: savedModelId } as ModelConfig, 'vlm')
            }
          }
        }
        setDialogOpen(false)
  ...
  const filteredModels = useMemo(() => {
    ...
    if (searchQuery.trim()) {
      ...
      result = result.filter(m =>
        m.name.toLowerCase().includes(q) || m.modelId.toLowerCase().includes(q)
      )
    }
    ...
    return result
  }, [models, searchQuery, filterProvider, dateFrom, dateTo])
  ...
  return (
    <div className="space-y-6">
      ...
      <StatsCardGrid columns={5}>
        <StatsCard
          value={models.length}
          label={isAr ? 'إجمالي النماذج' : 'Total Models'}
          icon={Bot}
        ...
        />
        <StatsCard
          value={models.find(m => m.id === effectiveLlmId)?.name || (isAr ? 'غير محدد' : 'Not set')}
          label={isAr ? 'LLM — تحليل النصوص' : 'LLM — Text Analysis'}
          icon={Star}
          iconClassName="bg-blue-100"
          className="border-blue-200 bg-gradient-to-b from-blue-50/50 to-white"
        />
        <StatsCard
          value={models.find(m => m.id === effectiveVlmId)?.name || (isAr ? 'غير محدد' : 'Not set')}
          label={isAr ? 'VLM — تحليل الصور والمستندات' : 'VLM — Image & Docs'}
          icon={Eye}
          iconClassName="bg-purple-100"
          className="border-purple-200 bg-gradient-to-b from-purple-50/50 to-white"
        />
        <StatsCard
          value={ollamaModels.length}
          label={isAr ? 'نماذج محلية' : 'Local Models'}
          icon={HardDrive}
      ...
      <Tabs defaultValue="configured" className="w-full">
        ...
        <TabsContent value="configured" className="mt-4 space-y-4">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            <Calendar className="w-3.5 h-3.5 text-ae-black-300" />
            <Label className="text-xs text-muted-foreground whitespace-nowrap">{isAr ? 'تاريخ الإنشاء' : 'Created Date'}</Label>
            ...
            {(searchQuery || filterProvider !== 'all' || dateFrom || dateTo) && (
              <div className="flex items-center gap-1.5 ms-auto">
                <SlidersHorizontal className="w-3 h-3 text-muted-foreground" />
                <span className="text-xs text-muted-foreground">
                  {isAr ? `${filteredModels.length} من ${models.length} نموذج` : `${filteredModels.length} of ${models.length} models`}
                </span>
              </div>
            )}
          </div>
          ...
          {loading ? (
            ...
            <EmptyState
              icon={Bot}
              title={isAr ? 'لا توجد نماذج' : 'No models found'}
              description={isAr ? 'أضف نماذج الذكاء الاصطناعي أو استخدم الإضافة الافتراضية' : 'Add AI models or seed defaults to get started'}
              action={
