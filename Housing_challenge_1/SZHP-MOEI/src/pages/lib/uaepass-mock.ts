...
export interface MockUserProfile extends UAEPassUserProfile {
  ...
  housingType?: 'villa' | 'apartment' | 'land'
  incomeStability?: 'stable' | 'unstable' | 'contract'
...
export const MOCK_USERS: Record<string, MockUserProfile> = {
  'ahmed-citizen': {
    ...
    spouseIncome: 12000,
    totalHouseholdIncome: 37000,
    numberOfChildren: 3,
    housingType: 'villa',
    incomeStability: 'stable',
    previousIncome: 26000,
    hasFamilyBook: true,
    isEmirati: true,
  },
  'fatima-citizen': {
  ...
  'mohammed-citizen': {
    ...
    maritalStatus: 'married',
    spouseIncome: 8000,
    totalHouseholdIncome: 23000,
    numberOfChildren: 3,
    housingType: 'villa',
    incomeStability: 'stable',
    previousIncome: 15500,
    hasFamilyBook: true,
    isEmirati: true,
  },
  'noura-citizen': {
    ...
    spouseIncome: 0,
    totalHouseholdIncome: 12000,
    numberOfChildren: 0,
    housingType: 'apartment',
    incomeStability: 'stable',
    previousIncome: 12000,
    hasFamilyBook: true,
    isEmirati: true,
  },
  'khalifa-citizen': {
  ...
  'mariam-citizen': {
    ...
    spouseIncome: 0,
    totalHouseholdIncome: 35000,
    numberOfChildren: 2,
    housingType: 'villa',
    incomeStability: 'stable',
    previousIncome: 36000,
    hasFamilyBook: true,
    isEmirati: true,
  },
  'resident-ineligible': {
    sub: 'UAEPASS/resident-denial-0001-0002-0003-000000000001',
    firstnameEN: 'Omar',
    lastnameEN: 'Khan',
    firstnameAR: 'عمر',
    lastnameAR: 'خان',
    fullnameEN: 'Omar Khan',
    fullnameAR: 'عمر خان',
    email: 'omar.khan@example.ae',
    mobile: '+971503219876',
    idn: '784-1991-7654321-0',
    ...
    role: 'citizen',
    monthlyIncome: 22000,
    employer: 'Gulf Facilities Services LLC',
    employerType: 'private',
    familySize: 3,
    hasActiveLoan: false,
    maritalStatus: 'married',
    spouseIncome: 7000,
    totalHouseholdIncome: 29000,
    numberOfChildren: 1,
    housingType: 'apartment',
    incomeStability: 'stable',
  ...
  'aisha-employee': {
    ...
    maritalStatus: 'married',
    spouseIncome: 8000,
    totalHouseholdIncome: 38000,
    numberOfChildren: 1,
    housingType: 'apartment',
    incomeStability: 'stable',
    previousIncome: 29000,
    hasFamilyBook: true,
    isEmirati: true,
  },
  // Admin profile
  'saeed-admin': {
    ...
    maritalStatus: 'married',
    spouseIncome: 12000,
    totalHouseholdIncome: 57000,
    numberOfChildren: 4,
    housingType: 'villa',
    incomeStability: 'stable',
    previousIncome: 44000,
    hasFamilyBook: true,
    isEmirati: true,
  },
}
...
export const MOCK_HOUSING_ASSISTANCE_FILES: Record<string, HousingAssistanceFile[]> = {
  ...
  '784-1990-1234567-1': [
    ...
    {
      housingAssistanceNumber: 'SZHP-2022-77881',
...
export function generateRandomMockUser(): MockUserProfile {
  ...
  const maritalStatusOptions: Array<'single' | 'married' | 'divorced' | 'widowed'> = ['single', 'married', 'married', 'married', 'divorced']
  const maritalStatus = maritalStatusOptions[Math.floor(Math.random() * maritalStatusOptions.length)]
  const spouseIncome = maritalStatus === 'married' ? 5000 + Math.floor(Math.random() * 15000) : 0
  const totalHouseholdIncome = monthlyIncome + spouseIncome
  const numberOfChildren = maritalStatus === 'single' ? 0 : Math.floor(Math.random() * 5)
  const housingTypeOptions: Array<'villa' | 'apartment' | 'land'> = ['villa', 'villa', 'apartment', 'apartment', 'land']
  const housingType = housingTypeOptions[Math.floor(Math.random() * housingTypeOptions.length)]
  const incomeStabilityOptions: Array<'stable' | 'unstable' | 'contract'> = employerType === 'government' ? ['stable', 'stable', 'stable', 'contract'] : ['stable', 'unstable', 'contract', 'unstable']
  const incomeStability = incomeStabilityOptions[Math.floor(Math.random() * incomeStabilityOptions.length)]
  const previousIncome = monthlyIncome + Math.floor(Math.random() * 3000) - 1000
