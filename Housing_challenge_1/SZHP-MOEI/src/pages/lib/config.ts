/**
 * Client-side config helper — reads system config from /api/system-config
 */
...
async function loadConfigs(): Promise<Record<string, string>> {
  ...
  try {
    const configs = await apiFetch('/api/system-config?public=true')
    ...
    console.warn('Could not load system configs from API, using defaults')
  }
