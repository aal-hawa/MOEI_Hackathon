...
function maskEmiratesId(eid: string | undefined | null): string {
  if (!eid) return '784-XXXX-XXXXXXX-X'
  // If already in a displayable format, try to mask the middle
...
const RISK_BADGE_STYLES: Record<string, string> = {
  low: 'bg-ae-green-50 text-ae-green-700 border-ae-green-200',
  medium: 'bg-ae-gold-50 text-ae-gold-700 border-ae-gold-200',
  high: 'bg-orange-50 text-orange-700 border-orange-200',
  critical: 'bg-ae-red-50 text-ae-red-700 border-ae-red-200',
}

const STATUS_BADGE_STYLES: Record<string, string> = {
  pending: 'bg-yellow-50 text-yellow-700 border-yellow-200',
  under_review: 'bg-blue-50 text-blue-700 border-blue-200',
  ai_assessed: 'bg-ae-gold-50 text-ae-gold-700 border-ae-gold-200',
  approved: 'bg-ae-green-50 text-ae-green-700 border-ae-green-200',
  rejected: 'bg-ae-red-50 text-ae-red-700 border-ae-red-200',
                  ...
                  >
                    ...
                    <div className="grid grid-cols-2 gap-x-4 gap-y-1.5 text-xs">
                      ...
                      <div className="flex items-center gap-1.5 flex-wrap">
                        <Badge
                          variant="outline"
                          className={`text-xs px-1.5 py-0 shrink-0 ${RISK_BADGE_STYLES[riskLevel] || RISK_BADGE_STYLES.medium}`}
                        >
                          {riskLabels[riskLevel] || riskLevel}
                        </Badge>
                        <Badge
                          variant="outline"
                          className={`text-xs px-1.5 py-0 shrink-0 ${STATUS_BADGE_STYLES[c.status] || ''}`}
                        >
                    ...
                    return (
                      ...
                      >
                        ...
                        <TableCell>
                          <Badge
                            variant="outline"
                            className={`text-xs px-1.5 py-0 ${RISK_BADGE_STYLES[riskLevel] || RISK_BADGE_STYLES.medium}`}
                          >
                            {riskLabels[riskLevel] || riskLevel}
                          </Badge>
                        </TableCell>
                        <TableCell>
                          <Badge
                            variant="outline"
                            className={`text-xs px-1.5 py-0 ${STATUS_BADGE_STYLES[c.status] || ''}`}
                          >
                            {statusLabels[c.status] || c.status}
                          </Badge>
                        </TableCell>
